...
async def generate_and_rm_group(
    ...
    # Per-trajectory exception isolation. Without return_exceptions=True a single
    # trajectory raising an unhandled exception cancels the whole gather and crashes
    # the entire rollout via CancelledError (which also swallows the root cause).
    # This is benign for plain RLVR rollouts where generate_and_rm never raises, but
    # agentic rollouts can raise after the custom generate() returns (e.g. trajectory
    # token-merge / prefix-drift edge cases outside generate()'s own try/except).
    # Log the real traceback and substitute an ABORTED resolved=False placeholder
    # (same fan-out list shape) so one bad sample cannot take down the batch.
