...
def parse_tool_uses(
    ...
    if tool_parser_name and tools_schema:
        ...
        if info is not None and info.tools_called:
            ...
            logger.debug("[agent.parsing] vllm parser: %d tool calls extracted", len(tool_uses))
        else:
            logger.debug("[agent.parsing] vllm parser: no tool calls found (info=%s)", info is not None)

    if not tool_uses and tools_schema:
        body_text, tool_uses = parse_xml_tool_uses(body_text, tools_schema)
        if tool_uses:
            logger.debug("[agent.parsing] XML fallback: %d tool calls", len(tool_uses))
