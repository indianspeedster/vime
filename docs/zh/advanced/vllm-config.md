...
![vllm-config 架构](../../_static/image/vllm_config.png)
...
配置文件是一个 YAML 文档，顶层 `vllm` 键包含一个模型定义列表：
...
每个模型都有独立的 router（默认使用 vllm-router）。
...
`--prefill-num-servers` 是启用 PD 分离的旧方式（它创建一个带有 prefill + decode 组的单模型）。`--vllm-config` 是更新、更灵活的方式。两者互斥。我们推荐所有新部署迁移到 `--vllm-config`。
