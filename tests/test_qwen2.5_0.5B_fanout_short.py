...
def execute():
    ...
    # handles the fan-out correctly via ``batched_async_rm`` on the flat
    # sibling list. ``--group-rm`` defers rm to ``generate_and_rm_group``
    # which assumes ``group`` is already flat ``list[Sample]`` — combining
    # it with a list-returning custom_generate yields a
